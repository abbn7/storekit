...
PG_BIN="$(dirname "$(find /usr/lib/postgresql -type f -name pg_ctl -print -quit)")"
...
if [ -z "${DATABASE_URL:-}" ]; then
  ...
  if ! runuser -u postgres -- "$PG_CTL" -D "$PGDATA" status >/dev/null 2>&1; then
    runuser -u postgres -- "$PG_CTL" -D "$PGDATA" -o "-c listen_addresses=127.0.0.1 -p $INTERNAL_PG_PORT" -w start
  fi
